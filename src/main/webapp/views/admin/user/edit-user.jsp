    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="mb-3 row">
                ...
                <div class="col-sm-10">
                    <input value="${user.name}" required class="form-control" type="tel" name="name"
                ...
                <div class="col-sm-10">
                    <select name="roles[]" class="form-select" multiple id="selectRole">
                        ...
                        <c:forEach var="role" items="${roles}">
                            ...
                            <option value="${role.id}" ${isSelected ? 'selected="selected"' : ''}>${role.name}</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
                ...
                <div class="col-sm-10">
                    <select name="departments[]" class="form-select" multiple id="selectDepartment">
                        ...
                        <c:forEach var="department" items="${departments}">
                            <c:set var="isSelected" value="false"/>
                            <c:forEach var="userDept" items="${user.departments}">
                                <c:if test="${department.id == userDept.id}">
                                    <c:set var="isSelected" value="true"/>
                                </c:if>
                            </c:forEach>
                            <option value="${department.id}" ${isSelected ? 'selected' : ''}>${department.name}</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
